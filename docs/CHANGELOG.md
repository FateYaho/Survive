...
### [DOC] 디자인 방향성 결정 후속 정리 + Survive 일시 동결 검토
- **무엇**: ADR-0004·0005 박은 후 디자인 풀이 후반에서 추가 결정 사항을 `docs/OPEN_ISSUES.md` "ADR-0006 박을 때 정식 옮김" 메모에 정리. 1티어/2티어 자원 구조 6종(1티어=나무·돌, 2티어=강화나무·강화돌·철·금), 캐릭터 성장 (iii) 모델 (1티어 메인 + 2티어 색깔 흡수), 영웅 장착 = 능력치 업글 통합, 랜덤 뽑기 폐기, FORGE/FACTORY (C) 의미 약화, 1티어=양/2티어=질 결 차이. P3에 합체 최상위 타워 후보 + 일회용 아이템 1티어 메인 재료 메모 추가. CLAUDE.md "현재 상태"에 디자인 방향성 결정 + Survive 일시 동결 검토 명시.
- **왜**: 디자인 인플레이션 인지 — 세션 진행 중 매 결정마다 새 시스템 추가되어 비개발자 첫 게임 스코프 한참 초과 우려. 본인 처음 직관(랜디 먼저 → 빠른 학습 사이클 → Survive 복귀) 합리성 검증됨. 결정 사항 휘발 방지 + Survive 동력 보존을 위해 정리. 다음 세션에서 (A) 랜디 시작 / (B) Survive 다시 결정.
- **파일**: `docs/OPEN_ISSUES.md`, `CLAUDE.md`
- **관련**: ADR-0004, ADR-0005, OPEN_ISSUES.md ("ADR-0006 박을 때 정식 옮김" 메모)

### [ARCH] ADR-0004 — 메타 진행 모델 (롤체식 수평 다양성)
- **무엇**: 메타 진행 X (영구 캐릭터·자원·언락 X, 매 판 리셋, BM 안 박음). 한 판 내 캐릭터 성장 O — 4계열 빌드 안에서 자원 분배 trade-off (타워 vs 캐릭터). 매 판 다양성 메카닉 = 바이옴(ADR-0005) + 사이클별 적(추후) + 자원 분배 변형.
- **왜**: 디자인 단계 방향성 결정. 원랜디 모델 검토했으나 RTS 시스템과 충돌 + 40~50종 SSS는 1인 첫 게임 스코프 초과. 롤체 모델이 디자인 의도(BM 안 박기, 매 판 다양성)와 일치.
- **파일**: `docs/adr/0004-meta-progression.md` (신규)
- **관련**: ADR-0004, GDD §0 Core Pillars, OPEN_ISSUES.md (4계열별 캐릭터 능력·강화 UI 미정)

### [ARCH] ADR-0005 — 바이옴 시스템 디자인
- **무엇**: 6개 결정 — (1) 주력+부산물+혼합 바이옴, (2) 풍부도 그라데이션+위치 랜덤(거리 보장), (3) 첫 마무리는 적이 NIGHT만 / 환경 위험은 Phase 3+, (4) EXPLORED→바닥 색깔 차별화, (5) 시드 기반 절차적 생성, (6) 결정 시점 자유 선택형.
- **왜**: ADR-0004의 다양성 메카닉 핵심 도구. 6종 터렛 풀 한계를 매 판 자원 분포 변동(상황 변수)으로 보완.
- **파일**: `docs/adr/0005-biome-system.md` (신규)
- **관련**: ADR-0005, ADR-0004, ADR-0002 (시야 시스템), OPEN_ISSUES.md (혼합 바이옴 조합·환경 위험·자원 풍부도 수치 미정)
